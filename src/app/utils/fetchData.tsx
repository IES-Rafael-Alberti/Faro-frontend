...
export  async function fetchProfileData (id: string, token: string = '') {
    console.log('fetchProfileData', id);
    
    try {
        return  await fetchData<ProfileInterface>(`${USER_BASIC_INFO_URL}${id}`, token)
    }catch (error) {
        console.error(`Error fetching profile data from ${USER_BASIC_INFO_URL}${id}:`, error)
        return Promise.reject(error)
    }
}
